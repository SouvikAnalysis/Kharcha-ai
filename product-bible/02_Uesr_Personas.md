# KHARCHA

## 02_USER_PERSONAS.md

---

# Persona 1 – The Salaried Professional (Primary Persona)

**Name:** Souvik

**Age:** 27

**Occupation:** IT Professional

**Monthly Income:** ₹37,500

### Goals

* Know where every rupee goes.
* Stay within a monthly budget.
* Save more every month.
* Build a habit of tracking expenses.

### Pain Points

* Salary disappears before month-end.
* Doesn't remember small daily expenses.
* Doesn't want to maintain a notebook.
* Existing apps feel complicated.
* Doesn't have time to enter lengthy details.

### App Usage

* Opens Kharcha 2–5 times a day.
* Records expenses immediately after spending.
* Checks dashboard at the end of the week and month.

---

# Persona 2 – The College Student

**Name:** Rahul

**Age:** 21

**Occupation:** Student

**Monthly Pocket Money:** ₹6,000

### Goals

* Manage pocket money.
* Avoid asking parents for extra money.
* Track food and travel expenses.

### Pain Points

* Doesn't know where money goes.
* Spends too much on food and outings.
* Never tracks expenses consistently.

### App Usage

* Uses Kharcha daily.
* Wants simple reports.
* Prefers reminders.

---

# Persona 3 – The PG/Hostel Resident

**Name:** Archit

**Age:** 24

**Occupation:** Software Engineer

**Monthly Income:** ₹42,000

### Goals

* Split rent and electricity.
* Track shared expenses.
* Know monthly living costs.

### Pain Points

* Confusion while splitting bills.
* Forgets shared expenses.
* No record of who paid what.

### Future Features

* Shared groups.
* Expense splitting.
* Settlement tracking.

---

# Persona 4 – The Young Working Professional

**Name:** Priya

**Age:** 29

**Occupation:** Marketing Executive

**Monthly Income:** ₹60,000

### Goals

* Budget monthly expenses.
* Save for travel.
* Avoid overspending on shopping.

### Pain Points

* Impulse purchases.
* Doesn't realize monthly shopping total.
* Wants spending insights.

### App Usage

* Checks dashboard every evening.
* Reviews monthly reports before salary day.

---

# Persona 5 – The Family Man

**Name:** Amit

**Age:** 38

**Occupation:** Private Employee

**Monthly Income:** ₹80,000

### Goals

* Manage family budget.
* Track school fees, groceries, EMI and bills.
* Increase monthly savings.

### Pain Points

* Many recurring expenses.
* Difficult to remember every payment.
* Needs reminders before due dates.

### Future Features

* Family accounts.
* Recurring expenses.
* Bill reminders.
* Savings goals.

---

# Common User Problems

Every Kharcha user usually has one or more of these problems:

* Doesn't know where money goes.
* Doesn't enjoy maintaining expense records.
* Wants a quick and simple app.
* Needs budget alerts.
* Wants useful reports instead of confusing data.
* Wants to build better financial habits.

---

# Design Principles Based on Personas

Every feature in Kharcha should follow these principles:

1. Expense entry should take less than 5 seconds.
2. The app should require as little typing as possible.
3. Important information should be visible at a glance.
4. The interface should be simple enough for first-time users.
5. Every screen should help users understand or control their money.
6. Users should never feel like they are doing accounting—they should feel like they are managing their life.

---

# Success Criteria

A successful Kharcha user should be able to answer these questions instantly:

* How much have I spent today?
* How much have I spent this month?
* Which category consumed the most money?
* How much budget is left?
* Am I spending more than last month?
* Where can I reduce unnecessary expenses?

If Kharcha helps users answer these questions effortlessly, it has fulfilled its purpose.
